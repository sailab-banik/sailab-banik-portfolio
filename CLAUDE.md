...
`--display-size` is derived, not chosen: `Sailab Banik` advances 3.758em in Bricolage at `wdth 78` with `-0.035em` tracking, and the content column is `100vw - 80px` until it caps at 1160px, so `26.3vw - 1.32rem` is the size at which the name spans the column. A media query re-solves it as `26.35vw - 0.79rem` below `md`, where the gutter is 24px instead of 40px. Both carry about 1% slack against font-rendering variance. Change the name, the width axis, or the tracking and both coefficients have to be re-measured or the hero stops fitting. Measure the *text*, not the block: the line is a block element that fills the column whether or not the type does, so `getBoundingClientRect()` on it always reports a perfect fit. Use a `Range` over its contents.
